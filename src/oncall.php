...
//  $_SESSION["global_gpid"]
  ...
  if($_SESSION["global_role"]!='אחראי עמדה')
		...
		echo'<h4> הנך מבצע שיבוץ כוננויות בעמדת  '.$emname.'  </h4></center>';
		}//end of if
		if($_SESSION["global_role"]=='אחראי עמדה')
		{
		...
		$emda=$_SESSION["global_gpid"];
					...
					foreach ($bigweek as $bigday){
						...
						$j=$j+1;
							 ...
							 echo '<br/> <br/>';
							$sqld="delete from Oncall where gpid=".$emda." and  oncall_date>='".$date_sunday."' and oncall_date<='".$date_saturday."' ;";
							...
							$j=0;
								...
								{
									$sql1 = "INSERT INTO Oncall VALUES('".$bigday[1][$i-$j]."','".$bigday[2][$i-$j]."','".$bigday[day]."','".$bigday[dat]."',".$emda.",".$_POST[$i].");";
									$result1 = mysqli_query($conn,$sql1);
								}
								$j=$j+5;
							}
							echo "<script type='text/javascript'>alert('.הזנת הכונניות הסתיימה בהצלחה')</script>";
						...
						foreach ($week as $day){
							...
							elseif($r5[3]==2)
								...
								$color="green";
							$sql7="select distinct F.user_id from Oncall F where F.oncall_start='".$r1[0]."' and F.oncall_end='".$r1[1]."' and F.oncall_day='".$day."' and oncall_date<='".$date_saturday."' and oncall_date>='".$date_sunday."' and F.gpid=".$emda." ; ";
							$res7=mysqli_query($conn,$sql7);
							while( $r7=mysqli_fetch_array($res7))
								$aid2=$r7[0];
							if($r5[0]!=$aid && $r5[0]==$aid2)

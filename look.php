		...
		$emda=$_SESSION["global_gpid"];
					...
					foreach ($bigweek as $bigday){
						...
						//	$result2 = mysqli_query($conn,$sqld);		
							...
							$i=0;
							$j=0;
					$get_sme_id="SELECT id FROM `ApprovedUser` WHERE gpid=$emda ORDER BY `ApprovedUser`.`role` DESC";
					...
					while( $id=mysqli_fetch_array($smes)){
						...
						foreach ($week as $day){
							...
							echo '<select name="'.$i.'">';
							$sql5="select distinct A.id,A.Fname,A.Lname,F.favshifts_rank from ApprovedUser A Join FavoriteShifts F on A.id=F.user_id where A.id=".$id[0]." and F.favshifts_start='".$r1[0]."' and F.favshifts_end='".$r1[1]."' and F.favshifts_day='".$day."' and A.gpid=".$emda." order by F.favshifts_rank desc; ";
